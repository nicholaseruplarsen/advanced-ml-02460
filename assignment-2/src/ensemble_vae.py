...
if __name__ == "__main__":
    ...
    if args.mode == "train":
        ...
        model.load_state_dict(torch.load(os.path.join(base_dir, args.experiment_folder, "model.pt")))
        model.eval()
        ...
        with torch.no_grad():
            ...
            save_image(
                torch.cat([data.cpu(), recon.cpu()], dim=0), os.path.join(base_dir, "reconstruction_means.png")
            )
        ...
        # we want 25 randoms pairs of data in the latent space. so take 25 datapoints from mnist, send through encoder, shuffle random and voila
        x = next(iter(mnist_test_loader))[0].to(device)
        z = model.encoder(x).mean.detach() # remember encoder outputs a gaussian, so we take the mean to get a point
        pairs = torch.randperm(len(z))[:50].reshape(25, 2)
        geodesics = []
        for i in tqdm(range(25), desc="Computing geodesics"):
            a = z[pairs[i, 0]]
            b = z[pairs[i, 1]]
            geodesics.append(compute_geodesic(a, b, model.decoder))
        
        # plot the stuff
        fig, ax = plt.subplots(figsize=(8, 8))

        # all latent points colored by class
        with torch.no_grad():
            all_x, all_y = [], []
        ...
        ax.set_title('Latent space with geodesics')
        plt.savefig(os.path.join(base_dir, args.experiment_folder, 'geodesics.png'), dpi=150)
        plt.show()
