...
class FlowPrior(nn.Module):
    """Normalizing flow prior for VAE, compatible with vae_bernoulli.py.

    Wraps Flow so that forward() returns self, exposing log_prob(z) and
    sample(sample_shape) for use in VAE.elbo() and VAE.sample().
    """

    def __init__(self, M, n_transformations=8, n_hidden=256):
        super().__init__()
        base = GaussianBase(M)
        mask = torch.zeros(M)
        mask[M // 2:] = 1
        ...
        for _ in range(n_transformations):
            ...
            scale_net = nn.Sequential(
                nn.Linear(M, n_hidden), nn.ReLU(),
                nn.Linear(n_hidden, M), nn.Tanh(),
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mask', type=str, default='half', choices=['half', 'random', 'checkerboard'], help='masking strategy for coupling layers (default: %(default)s)')
    parser.add_argument('--num-transformations', type=int, default=5, metavar='N', help='number of coupling layer transformations (default: %(default)s)')
    parser.add_argument('--num-hidden', type=int, default=8, metavar='N', help='hidden units in coupling networks (default: %(default)s)')
    ...
    import ToyData
    if args.data == 'mnist':
        class _Unlabeled(torch.utils.data.Dataset):
        ...
        train_loader = torch.utils.data.DataLoader(
            _Unlabeled(datasets.MNIST('data/', train=True, download=True, transform=mnist_transform)),
            batch_size=args.batch_size, shuffle=True)
        test_loader = torch.utils.data.DataLoader(
            _Unlabeled(datasets.MNIST('data/', train=False, download=True, transform=mnist_transform)),
    ...
    # Initialize base mask based on masking strategy
    if args.mask == 'checkerboard' and D == 784:
        mask = torch.tensor([(i // 28 + i % 28) % 2 for i in range(D)], dtype=torch.float)
    else:
        mask = torch.zeros(D)
        mask[D // 2:] = 1
    ...
    if args.mode == 'train':
        ...
        if args.data == 'mnist':
            save_image(samples[:64].clamp(0, 1).view(-1, 1, 28, 28), args.samples, nrow=8)
        else:
